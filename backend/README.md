...
### Deviation 10 — `PROVIDER_CONFIG["groq_gpt_oss"]["use_for"]` gains `"remediation"`; `gemini_flash_thinking` loses it

**Bible says:** Section 8.1 lists `"remediation"` under the Google thinking entry (`gemini_flash_thinking["use_for"]`).

**Implemented:** Quick task 260826-rsw moved `"remediation"` from `gemini_flash_thinking["use_for"]` to `groq_gpt_oss["use_for"]`, alongside the `"narration"` key quick task 260826-p1q already added there. Both halves of the edit are required together: `select_provider()` returns the FIRST `PROVIDER_CONFIG` entry whose `use_for` contains the task, and `gemini_flash_thinking` is declared before `groq_gpt_oss` — appending the key to Groq alone, without removing it from Google, would have been a silent no-op (routing stays on Gemini, every test still passes, latency unchanged). `"orchestrator"` and `"synthesis"` remain on `gemini_flash_thinking`, untouched. `_build_openai_compatible_request` (shared by Groq, DeepSeek, and OpenRouter) now accepts and honours a `json_output` flag, adding the standard `response_format: {"type": "json_object"}` field for every provider it serves when set, and sizing Groq's `max_completion_tokens` budget by output shape — `GROQ_MAX_COMPLETION_TOKENS_DEFAULT` (512, unchanged from before) without JSON output, `GROQ_MAX_COMPLETION_TOKENS_JSON` (2048) with it. `app/agents/a7_remediation.py`'s `synthesize_capa` wraps its `call_llm` invocation in a 6.0-second total wall-clock ceiling (`A7_REMEDIATION_CEILING_SECONDS`) via `asyncio.wait_for`, falling back to `_deterministic_capa` — with a logged warning — on ceiling breach, router degradation, or a parse failure.

**Why:** CAPA generation against `gemini_flash_thinking` measured ~18s per call with no wall-clock ceiling, the same class of cold-path latency quick task 260826-p1q already fixed for A2's narration path on every call site except this one. `openai/gpt-oss-120b` (`groq_gpt_oss`) is materially faster and, under this fix's JSON-mode wiring, produces the same well-formed four-field narrative Gemini did.

**Evidence:** `[VERIFIED: live measurement during 260826-rsw]` — a real `generate-capa` request against a C1-eligible seeded finding (`A2-ANNEX11-S11-PE-001-PE-2024-01`, `GXP-MFG-DEMO-01`) completed in **1.447s** (vs. the 18s baseline) with a model-authored, non-template narrative. A direct `call_llm(task="remediation", json_output=True)` probe against the same live key completed in 0.978s, reporting `completion_tokens=264` against the 2048 cap (`finish_reason="stop"`, no truncation) — see `260826-rsw-SUMMARY.md` for the full measurement and the token-budget headroom this implies.

**Scope:** `backend/app/llm_router.py`'s `PROVIDER_CONFIG["gemini_flash_thinking"]["use_for"]` / `PROVIDER_CONFIG["groq_gpt_oss"]["use_for"]`, `_build_openai_compatible_request`'s new `json_output` parameter and Groq completion-cap constants, and `backend/app/agents/a7_remediation.py`'s `synthesize_capa` wall-clock ceiling. Routed to **SENT-7-05**.

**Pre-existing documentation gap, noted but not fixed here (Rule 7):** `llm_router.py`'s own module docstring already references "Deviation 10", "Deviation 11", and "Deviation 12" for three separate, already-shipped corrections (the retired Gemini 2.5 flash model id, the `gemini_flash_fast` thinking-budget fix, and the retired Llama 3.3 70B model id) that have no corresponding heading anywhere in this file — those numbers were assigned in the code comment before this section was ever backfilled for them. Quick task 260826-p1q's `"narration"` key addition to `groq_gpt_oss["use_for"]` was similarly never given its own README heading. This section's own new heading is numbered by the highest EXISTING heading in this file (9) plus one, per this task's own scoping — it deliberately does not attempt to renumber or backfill the code docstring's numbers, which belongs to the SENT-7-05 Bible-reconciliation pass, not to this quick task.
...
### Deviation 13 — `call_llm()` cascades through all four providers on failure, not one; `groq_llama` renamed `groq_gpt_oss`

**Bible says:** `AegisX-AI-Project-Bible-v6.md` Section 8.2 specifies a single-hop cascade — a failing provider falls back exactly once to `openrouter_fallback`, which is either usable or the call degrades.

**Implemented:** `backend/app/llm_router.py`'s `call_llm()` now walks a new `FALLBACK_CASCADE` tuple (`gemini_flash_fast`, `deepseek_r1`, `groq_gpt_oss`, `openrouter_fallback`) in order on any cascadable failure (missing key, timeout, HTTP 429/5xx), skipping any entry whose underlying `provider` field was already attempted — so a Google-family retry never fires twice against the same key/quota — until one succeeds or `openrouter_fallback` itself also fails. A non-cascadable error (any other 4xx) still returns a degraded response immediately, unchanged from before. The two callers that wrap `call_llm()` in their own `asyncio.wait_for` — `a2_compliance.narrate_gap` and `a7_remediation.synthesize_capa` — had their outer wall-clock ceilings widened (3.0s → 13.0s narration; 6.0s → 18.0s remediation) to sit above the new worst-case sum of every hop's per-hop budget, not below it; the old ceilings reused the same value for both the per-hop budget and the total budget, which correctly bounded a 1-primary + 1-fallback cascade but would truncate a 4-hop cascade mid-flight. `PROVIDER_CONFIG`'s `"groq_llama"` key is renamed `"groq_gpt_oss"` throughout the codebase (config, both callers, both test files) — cosmetic only, matching Deviation 10/12's already-corrected `model` field (`openai/gpt-oss-120b`) instead of the retired Llama 3.3 70B id the old key name still implied.

**Why:** User directive 2026-08-29, observed live: a real demo session hit repeated Groq `429`s (rate limit) with `GEMINI_API_KEY`/`DEEPSEEK_API_KEY`/`OPENROUTER_API_KEY` all genuinely configured and idle. The old single-hop cascade to `openrouter_fallback` should have recovered, but `a2_compliance.narrate_gap`'s outer 3.0s `wait_for` — sized for exactly one fallback hop — fired mid-flight during the OpenRouter attempt, cancelling it before it could complete and landing on `deterministic-fallback` despite a working provider being one hop away. Widening the cascade to try every configured provider, not just the Bible's single named fallback, uses the capacity that was already paid for and sitting idle.

**Scope:** `backend/app/llm_router.py` (`FALLBACK_CASCADE`, `call_llm()`, `_classify_failure()`, the `groq_llama`→`groq_gpt_oss` key rename), `backend/app/agents/a2_compliance.py` (`NARRATION_PER_HOP_TIMEOUT_SECONDS`, `NARRATION_CEILING_SECONDS`), `backend/app/agents/a7_remediation.py` (`A7_REMEDIATION_PER_HOP_TIMEOUT_SECONDS`, `A7_REMEDIATION_CEILING_SECONDS` widened), `backend/tests/conftest.py` (new autouse `_isolate_llm_provider_keys` fixture — the multi-hop cascade surfaced a pre-existing test-isolation gap: `load_dotenv()` makes real `.env` keys ambient in `os.environ` unless a test explicitly clears them, previously harmless because the old single-hop cascade only ever touched two of the four providers), and `backend/tests/test_llm_router.py` / `backend/tests/test_a7_remediation.py` (key rename only). Routed to **SENT-7-05**.

### Deviation 14 — `call_embedding()`/`call_embeddings_batch()` retry HTTP 429 with backoff instead of failing the whole document

**Bible says:** No corresponding Bible text — the embedding module itself (`backend/app/retrieval/embeddings.py`) is phase 06.1 net-new surface area (Deviation 14 in the CODE docstring's own numbering, distinct from this file's independently-incrementing README numbering per the established convention noted at Deviation 11/12 above), so there is no prior Bible behavior to deviate from. Recorded here anyway because it changes previously-shipped 06.1 behavior in a way worth a permanent note.

**Implemented:** `call_embedding()` and `_call_batch_group()` (`backend/app/retrieval/embeddings.py`) both retry up to `EMBEDDING_MAX_RETRIES` (3) times with backoff specifically on an HTTP `429` response — 1s/2s/4s exponential backoff by default, or the exact delay from a `Retry-After` response header when the provider sends one (authoritative over our own guess). Every other failure path (timeout, non-429 status, transport error, malformed response body) is unchanged: degrades on the first attempt, no retry.

**Why:** A real ~105-chunk DOCX upload reliably tripped the hosted Gemini embedding API's rate limit — the batch call (32 chunks/request) failed on the final small group, fell back to 9 sequential per-chunk calls, and 4 of those hit real `429`s with zero retry anywhere in the module. `index_document()`'s all-or-nothing write (`backend/app/retrieval/ingest.py`, correct-by-design per D-09 — never persist a partial/fabricated index) then discarded the entire document, including the 96 chunks that had already embedded successfully in earlier batch groups. `429` is specifically a "try again shortly" signal, unlike every other status this module handles, so it is the one case worth retrying rather than degrading immediately.

**Scope:** `backend/app/retrieval/embeddings.py` only (`EMBEDDING_MAX_RETRIES`, `EMBEDDING_RETRY_BASE_DELAY_SECONDS`, `_retry_delay_seconds()`, and the retry loop wired into both `call_embedding()` and `_call_batch_group()`) and `backend/tests/test_retrieval_embeddings.py` (4 new regression tests). `ingest.py`'s all-or-nothing write behavior is unchanged and correct — this deviation makes the transient case succeed rather than relaxing that guarantee. Routed to **SENT-7-05**.

### Deviation 15 — `gemini_flash_thinking`, `gemini_flash_fast`, and `deepseek_r1` removed from `PROVIDER_CONFIG`; replaced by local `ollama_qwen`, which now leads `FALLBACK_CASCADE`

**Bible says:** Section 8.1's `PROVIDER_CONFIG` names `gemini_flash_thinking` (Gemini, `orchestrator`/`synthesis`), `gemini_flash_fast` (Gemini, `compliance`/`knowledge`/`change`/`rerank`), and `deepseek_r1` (DeepSeek, `risk_assessment`) as three of the router's four provider entries. Section 8.2 assumes every entry is a hosted API.

**Implemented:** `backend/app/llm_router.py`'s `PROVIDER_CONFIG` no longer has `gemini_flash_thinking`, `gemini_flash_fast`, or `deepseek_r1` entries at all. A new `ollama_qwen` entry (`provider: "ollama"`, model `qwen2.5:7b-instruct`, `base_url: "http://127.0.0.1:11434/v1"`, no `api_key_env` — local, unauthenticated) now serves all seven tasks those three entries used to split between them (`orchestrator`, `synthesis`, `compliance`, `knowledge`, `change`, `rerank`, `risk_assessment`), and leads `FALLBACK_CASCADE` ahead of the unchanged `groq_gpt_oss` and `openrouter_fallback` entries. `ollama_qwen` reuses the same OpenAI-compatible request/response builders Groq/DeepSeek/OpenRouter already shared (`_build_openai_compatible_request`/`_parse_openai_compatible_response`) — mechanically a fifth OpenAI-compatible provider, not new request logic. Embeddings moved the same way: `backend/app/retrieval/embeddings.py` now routes `embed_document`/`embed_query` to a local `nomic-embed-text` Ollama entry instead of Gemini's `gemini-embedding-001`, and the Qdrant collection was wiped and rebuilt rather than mixed across embedding spaces.

**Why:** Live, confirmed quota exhaustion on both replaced providers on 2026-08-31 — Gemini hit real `429`s on both embeddings and `gemini-3.6-flash` text-gen, and DeepSeek returned a live `402 Payment Required` (no billing configured, not transiently rate-limited). Both were structurally broken, not degraded. `groq_gpt_oss` and `openrouter_fallback` had zero failures that session and were left unchanged — this migration removes only the two providers with live evidence against them, not a wholesale rewrite of the cascade. See `LOCAL-MODELS-BUILD-MAP.md` (repo root) for the full investigation, hardware constraints (RTX 4060 8GB VRAM), and the staged Day 1 (embeddings)/Day 2 (LLM ops) rollout this deviation completed.

**Evidence:** `[VERIFIED: LOCAL-MODELS-BUILD-MAP.md — live 429/402 responses recorded 2026-08-31]`; `ollama_qwen` GPU-verified at 1.74s warm / 7.6s cold per `llm_router.py`'s own module comment.

**What this does not change.** C1, C2, and C3 make zero LLM calls by design (Bible §1.3) and are untouched by this migration — deterministic-first enforcement is unaffected either way. `groq_gpt_oss` and `openrouter_fallback`'s config, and the multi-hop cascade behavior from Deviation 13, are unchanged.

**Scope:** `backend/app/llm_router.py`'s `PROVIDER_CONFIG` and `FALLBACK_CASCADE`, `backend/app/retrieval/embeddings.py`'s embedding provider routing, plus the corresponding `respx`-mocked test files updated to target `localhost:11434` shapes. Not yet reconciled into the Bible's own Section 8.1/8.2 text — this README entry is the interim record per CLAUDE.md's drift rule. Routed to **SENT-7-05**.
