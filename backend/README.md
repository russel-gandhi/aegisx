...
**Scope:** `backend/app/llm_router.py`'s `PROVIDER_CONFIG["gemini_flash_thinking"]["use_for"]` / `PROVIDER_CONFIG["groq_llama"]["use_for"]`, `_build_openai_compatible_request`'s new `json_output` parameter and Groq completion-cap constants, and `backend/app/agents/a7_remediation.py`'s `synthesize_capa` wall-clock ceiling. Routed to **SENT-7-05**.
